$(document).ready(function() {  
    $.getJSON("http://ws.audioscrobbler.com/2.0/?method=user.gettopalbums&user=rj&api_key=4a9f5581a9cdf20a699f540ac52a95c9&limit=10&format=json&callback=?", function(json) {  
        ...
        $.each(json.topalbums.album, function(i, item) {  
			...
			html +="<div class='col-md-2' style='margin-left:40px;margin-top:20px'><a href='album.php?album="+item.name+"&artist="+a.name+"' target='_blank'><img src="+str +"></a><br><a style='margin:0 auto;' href='album.php?album="+item.name+"&artist="+a.name+"'>"+item.name+"</a></div>"	
        });  
        $('#top_alb').append(html);  
    });  
})