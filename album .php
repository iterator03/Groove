...
<body>
    ...
    <script>
		$(document).ready(function() {  
			...
			$.getJSON("http://ws.audioscrobbler.com/2.0/?method=album.getinfo&artist="+ar+"&album="+al+"&api_key=4a9f5581a9cdf20a699f540ac52a95c9&limit=10&format=json&callback=?", function(json) {  
				var html = '';
				var	str='';	
				var sln = 0;
				var j='';
				...
				$.each(json.album.tracks.track, function(i, item) {  
					...
					html += "<a href='practice.php?song="+item.name+"&mbid="+item.mbid+"'>"+item.name+"</a><a href='follow_Song.php?mbid="+item.mbid+"&name="+item.name+"' style='margin-left:60px'>Like this Song</a><hr>"; 
